...
$(function(){
    ...
    socket.on("viewGroupTimetable", function (data) {
     ...
     $("#approveTT").on("click",function(){
         user = "COD";
         var data = {
             group:group,
             dept:"CS",
             status:"Approved Timetable"
         }
          socket.emit("updateTimetableStatus",data);
     });
     $("#requestMod").on("click",function(){
         user = "COD";
         var data = {
             group:group,
             dept:"CS",
    ...
    for( var i = 0; i < 12; i++ ){          
          ...
          $(tableDef).addClass("container");
            ...
            $(tableDef).append("");
               $(tableDef).append("<br>");
              $(tableDef).append("<div>"+
                       " <div  style='height:1px; float:left; width:24px'>JJJ</div>"+
                        "<div class='e-gridtreeexpand'>HHH</div>"+
                        "<div style='float: left;width:10px;height:1px;'>LL</div>"+
