...
import com.slife.chris.studentlife.lecturer.LecturerUnitsActivity;
...
public class LecturerLoginActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        myPrefences = new PreferencesClass(LecturerLoginActivity.this);
        mSocket = SocketInstance.getSocket();
        mSocket.connect();

        mSocket.on("signInUser", onSignInUser);
        mSocket.on("login", onloggedIn);
        mSocket.on("checkLecturerDetails", onGetLecturerDetails);
                ...
                public void run() {
                    ...
                    try {
                        ...
                        switch (action) {
                            case "signInUser":
                             if (result == 0) {
                                    Toast.makeText(LecturerLoginActivity.this, "Invalid Credentials !", Toast.LENGTH_SHORT).show();
                                } else if (result == 1) {
                                    ...
                                    Toast.makeText(LecturerLoginActivity.this, "Welcome Back ! ", Toast.LENGTH_LONG).show();
                                    JSONObject credentials = new JSONObject();
                                    try {
                                 ...
                                 Toast.makeText(LecturerLoginActivity.this, "Please Complete Profile ! ", Toast.LENGTH_SHORT).show();
                                 String user_id = data.getString("user_id");
                                 myPrefences.storeUserId(user_id);
                                 myPrefences.storePhoneNo(phoneNo);
    ...
    private Emitter.Listener onGetLecturerDetails = new Emitter.Listener() {
        @Override
        public void call(final Object... args) {
            final JSONObject data = (JSONObject) args[0];
                ...
                public void run() {
                    ...
                    try {
                        if(data.getInt("success") == 1){
                            String lecturer_id = data.getString("checkLecturerDetails");
                            if (TextUtils.isEmpty(myPrefences.getLecturerId())) {
                                myPrefences.storeLecturerId(lecturer_id);
                            }
                        }
                        Intent intent = new Intent(LecturerLoginActivity.this, LecturerUnitsActivity.class);
                        startActivity(intent);
    ...
    class Login extends AsyncTask<JSONObject, String, String> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            progressDialog = new ProgressDialog(LecturerLoginActivity.this,
        ...
        protected String doInBackground(JSONObject... args) {
            ...
            UserDb userDbDb = new UserDb(LecturerLoginActivity.this);
            try {
            ...
            try {
                ...
                mSocket.emit("checkLecturerDetails",object);
            } catch (JSONException e) {
                e.printStackTrace();
            }
